...
Add this to your repo's `AGENTS.md` or equivalent agent instruction file:
...
Do not use `git push` directly for agent-assisted commits. `percentvibed push` sets the environment flag expected by the pre-push hook and hides local `.percentvibed/` state after a successful push.

Never commit raw prompts, raw logs, `.env` files, secrets, credentials, or private session data. Only commit sanitized output generated by `percentvibed capture`.
````

### 4. Optionally document it for humans

Add a shorter note to `CONTRIBUTING.md` or your project README:
